...
class pdf_pickup_soleil extends ModelePDFPickup
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$outputlangs->loadLangs(array("main", "pickup@pickup", "dict", "companies"));
		...
		if ($conf->pickup->dir_output)
		{
			...
			if ($object->specimen)
			{
				$dir = $conf->pickup->dir_output;
				$file = $dir."/SPECIMEN.pdf";
			}
			else
			{
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->pickup->dir_output."/".$object->element."/".$objectref;
			...
			if (file_exists($dir))
			{
				...
				$hookmanager->initHooks(array('pickuppdfgeneration', 'pdfgeneration'));
				...
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("PickupCard"));
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					if ($valide > 0 || $object->specimen)
					{
						...
						$desc = dol_htmlentitiesbr($objectligne->description, 1); // FIXME: html or text field?? correct?
		...
		{
			$this->error = $langs->trans("ErrorConstantNotDefined", "PICKUP_OUTPUTDIR");
			return 0;
		}
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// Load traductions files required by page
		$outputlangs->loadLangs(array("main", "dict", "companies", "pickup@pickup"));
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Date")." : ".dol_print_date($object->date_pickup, "day", false, $outputlangs, true), '', 'R');
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		return pdf_pagefoot($pdf, $outputlangs, 'PICKUP_FREE_TEXT', $this->emetteur, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
	}
}
