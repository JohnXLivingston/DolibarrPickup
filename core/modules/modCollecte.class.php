...
/**
 * 	\defgroup   collecte     Module Collecte
 *  \brief      Collecte module descriptor.
 *
 *  \file       htdocs/collecte/core/modules/modCollecte.class.php
 *  \ingroup    collecte
 *  \brief      Description and activation file for module Collecte
 */
include_once DOL_DOCUMENT_ROOT .'/core/modules/DolibarrModules.class.php';

/**
 *  Description and activation class for module Collecte
 */
class modCollecte extends DolibarrModules
{
    /**
    ...
    public function __construct($db)
    {
        ...
        // Module label (no space allowed), used if translation string 'ModuleCollecteName' not found (Collecte is name of module).
        $this->name = preg_replace('/^mod/i', '', get_class($this));
        // Module description, used if translation string 'ModuleCollecteDesc' not found (Collecte is name of module).
        $this->description = "CollecteDescription";
        // Used only if file README.md and README-LL.md not found.
        $this->descriptionlong = "Collecte description (Long)";
        ...
        // Key used in llx_const table to save module status enabled/disabled (where COLLECTE is value of property name of module in uppercase)
        ...
        $this->module_parts = array(
            ...
            'css' => array(
                //    '/collecte/css/collecte.css.php',
            ),
            // Set this to relative path of js file if module must load a js on all pages
            'js' => array(
                //   '/collecte/js/collecte.js.php',
            ),
        ...
        $this->langfiles = array("collecte@collecte");
        ...
        //$this->automatic_activation = array('FR'=>'CollecteWasAutomaticallyActivatedBecauseOfYourCountryChoice');
        //$this->always_enabled = true;								// If true, can't be disabled

        // Constants
        // List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
        // Example: $this->const=array(1 => array('COLLECTE_MYNEWCONST1', 'chaine', 'myvalue', 'This is a constant to add', 1),
        //                             2 => array('COLLECTE_MYNEWCONST2', 'chaine', 'myvalue', 'This is another constant to add', 0, 'current', 1)
        // );
        $this->const = array(
            // 1 => array('COLLECTE_MYCONSTANT', 'chaine', 'avalue', 'This is a constant to add', 1, 'allentities', 1)
        );
        ...
        if (! isset($conf->collecte) || ! isset($conf->collecte->enabled)) {
            $conf->collecte=new stdClass();
            $conf->collecte->enabled=0;
        }
        ...
        $this->dictionaries=array(
            ...
            'tabcond'=>array($conf->collecte->enabled,$conf->collecte->enabled,$conf->collecte->enabled)
        );
        */

        // Boxes/Widgets
        // Add here list of php file(s) stored in collecte/core/boxes that contains a class to show a widget.
        $this->boxes = array(
            //  0 => array(
            //      'file' => 'collectewidget1.php@collecte',
            //      'note' => 'Widget provided by Collecte',
        ...
        $this->cronjobs = array(
            ...
            //      'label' => 'MyJob label',
            //      'jobtype' => 'method',
            //      'class' => '/collecte/class/collecte.class.php',
            //      'objectname' => 'Collecte',
            ...
            //      'unitfrequency' => 3600,
            //      'status' => 0,
            //      'test' => '$conf->collecte->enabled',
            //      'priority' => 50,
            //  ),
        );
        // Example: $this->cronjobs=array(
        //    0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->collecte->enabled', 'priority'=>50),
        //    1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'$conf->collecte->enabled', 'priority'=>50)
        // );
        ...
        $r++;
        $this->rights[$r][0] = $this->numero + $r;	// Permission id (must not be already used)
        $this->rights[$r][1] = 'Create/Update objects of Collecte';	// Permission label
        $this->rights[$r][4] = 'write';				// In php code, permission will be checked by test if ($user->rights->collecte->level1->level2)
        $this->rights[$r][5] = '';				    // In php code, permission will be checked by test if ($user->rights->collecte->level1->level2)
        $r++;
        $this->rights[$r][0] = $this->numero + $r;	// Permission id (must not be already used)
        ...
        $this->menu[$r++]=array(
            ...
            'mainmenu'=>'collecte',
            'leftmenu'=>'',
            'url'=>'/collecte/collecteindex.php',
            'langs'=>'collecte@collecte',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'position'=>1000+$r,
            'enabled'=>'$conf->collecte->enabled',  // Define condition to show or hide menu entry. Use '$conf->collecte->enabled' if entry must be visible if module is enabled.
        ...
        */
        ...
        $this->menu[$r++]=array(
                                ...
                                'position'=>1100+$r,
                                'enabled'=>'$conf->collecte->enabled',  // Define condition to show or hide menu entry. Use '$conf->collecte->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
                                //'perms'=>'$users->rights->collecte->configure',               // Use 'perms'=>'$user->rights->collecte->level1->level2' if you want your menu with a permission rules
                                'perms' => '1',
                                'target'=>'',
                                'user'=>2);				                // 0=Menu for internal users, 1=external users, 2=both
    ...
    {
        $result=$this->_load_tables('/collecte/sql/');
        if ($result < 0) return -1; // Do not activate module if error 'not allowed' returned when loading module SQL queries (the _load_table run sql with run_sql with the error allowed parameter set to 'default')

        // Create extrafields during init
        //include_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
        //$extrafields = new ExtraFields($this->db);
        //$result1=$extrafields->addExtraField('myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'collecte@collecte', '$conf->collecte->enabled');
        //$result2=$extrafields->addExtraField('myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'collecte@collecte', '$conf->collecte->enabled');
        //$result3=$extrafields->addExtraField('myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'collecte@collecte', '$conf->collecte->enabled');
        //$result4=$extrafields->addExtraField('myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', 0, 0, '', '', 'collecte@collecte', '$conf->collecte->enabled');
        //$result5=$extrafields->addExtraField('myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'collecte@collecte', '$conf->collecte->enabled');
