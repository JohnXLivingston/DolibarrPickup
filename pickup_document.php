...
/**
 *  \file       pickup_document.php
 *  \ingroup    pickup
 *  \brief      Tab for documents linked to Pickup
 */
...
$hookmanager->initHooks(array('pickupdocument','globalcard'));     // Note that conf->hooks_modules contains array
...
//if ($id > 0 || ! empty($ref)) $upload_dir = $conf->pickup->multidir_output[$object->entity?$object->entity:1] . "/pickup/" . dol_sanitizeFileName($object->id);
...
$title=$langs->trans("Pickup").' - '.$langs->trans("Files");
$help_url='';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("Pickup"), -1, $object->picto);
	...
	$relativepathwithnofile='pickup/' . dol_sanitizeFileName($object->ref).'/';
